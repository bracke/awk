...
| `AWK-COMPAT-REDIRECTION-001` | Redirection | Supported with documented difference | `awklib 0.1.0` Output_Files API; `Awk_CLI.Execution.Supports_Redirection_Append_Mode = False` | `context append redirection limitation` | `awklib` returns captured final redirected output without exposing whether a target used `>` or `>>`, so the CLI materializes final files after interpretation and cannot preserve append intent without parsing AWK source. |
| `AWK-COMPAT-STREAMING-001` | Input | Supported with documented difference | `awklib 0.1.0` Run API; `Awk_CLI.Execution.Supports_Streaming_Execution = False` | `awklib execution adapter` | The resolved interpreter API accepts complete source and input and returns captured output, so V1 execution is memory-oriented rather than true streaming. The CLI does not emulate streaming with a custom AWK record loop. |
