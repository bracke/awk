...
| `AWK-COMPAT-ASSIGNMENT-001` | Command line | Supported with documented difference | `awklib 0.1.0` Arguments API | `context runtime assignment limitation` | `awklib` does not expose exact positional runtime-assignment execution. The CLI preserves operand spelling/order in `ARGV` but does not emulate a custom record loop. |
