...
| --- | --- | --- | --- | --- | --- |
| AWK-COMPAT-REGEX-001 | regular expressions | Supported | resolved awklib 0.1.0 behavior | awk process : process regex arithmetic builtins | Regular-expression integration follows resolved awklib behavior. |
...
| AWK-COMPAT-GETLINE-002 | getline | Supported | resolved awklib 0.1.0 behavior | awk process : process command getline | `command | getline` is handled through the awklib command callback. |
...
| AWK-COMPAT-PRINTF-001 | output formatting | Supported | resolved awklib 0.1.0 behavior | awk process : process printf formatting | `printf %c` field-width behavior follows resolved awklib. |
